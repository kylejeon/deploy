...
### 3-5. 포트포워딩 — 밖에서 타겟 UI 열기

설치가 끝나면 타겟에 traefik 이 `8000`(프론트) · `8001`(Temporal) · `8002`(WebPACS) ·
`8003`(Grafana) 를 연다. 그런데 이 주소들은 **사내망 안에서만** 닿는다.
...
- **포트**는 위 네 개만. 설치가 실제로 여는 traefik hostPort 다. 22 든 3306 이든
  목록 밖이면 거절한다 — 아니면 로그인한 사람이 병원망을 훑을 수 있다.
