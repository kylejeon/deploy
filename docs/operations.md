...
### 5-1-2. 설치 코드 브랜치 (hub-provisioning)

작업은 시작 직전에 hub-provisioning 을 **고른 브랜치의 최신**으로 맞춘다.
기본은 `main` 이고, 새 설치·패치 화면의 `설치 코드` 에서 바꾼다.

**언제 바꾸나**: 아직 `main` 에 들어가지 않은 기능을 시험할 때다. 예를 들어
`configure --only` 는 `dev` 에만 있어서, main 으로 돌리면 hubctl 이
`unknown option: --only` 로 죽는다 (2026-09-04 실제 사례).

- 고른 브랜치는 **작업마다 기록된다.** main 이 아니면 작업 상세에 노란 글씨로
  `설치 코드: dev` 가 뜨고, 명령 미리보기 밑에도 적힌다. 실행 로그 첫 줄에도
  `hub-provisioning 을 dev 최신으로 맞추는 중` 이 남는다.
- **다음 작업은 다시 main 이 기본이다.** 화면에 남아 붙지 않는다 — 시험용
  브랜치로 실서버를 깔아버리는 사고를 막기 위해서다.
- 브랜치 이름은 모양만 검사한다 (`-` 로 시작하거나 공백·`..` 이 섞이면 거부).
  select 에 없는 브랜치가 필요하면 `console.html` 의 `f-branch`·`p-branch` 에
  `<option>` 을 더한다.
...
받지 않고 **타겟에서 직접 읽어** 채운다 (`kubectl get gitrepository -n flux-system
hub-deploy -o jsonpath='{.spec.ref}'`, sudo 없이 사용자 kubeconfig 로).

- 고른 서버가 여럿이고 서로 다른 ref 를 보고 있으면 실행을 막는다 —
  `-e hub_deploy_ref` 는 실행당 하나뿐이라 한 번에 못 돌린다.
- 읽어온 값은 서버 정보에 저장되고 **읽은 시각도 함께 남는다.** 서버는 콘솔
  밖에서도 바뀌므로 '그때 그랬다' 이지 '지금 그렇다' 가 아니다.
...
### 부록 — 맥북에서 배포하기 (`ops/deploy.sh`)

맥북은 개인 장비라 `~/.ssh/known_hosts` 가 주기적으로 초기화된다
(2026-09-04, 2026-09-09 두 번 실측 — 그때마다 `Host key verification failed` 로
배포가 멎었다). 맥미니는 그럴 일이 없다.

그래서 맥미니의 호스트 키를 **저장소 안**(`ops/known_hosts`)에 고정해 두고
배포는 그것만 본다. `~/.ssh` 는 건드리지 않는다 — 호스트 공개키는 비밀이 아니다.

```bash
./ops/deploy.sh        # claude 브랜치 push + 맥미니 동기화 + 반영된 커밋 출력
```

`StrictHostKeyChecking=yes` 는 그대로 둔다. 키가 **진짜로** 바뀌면 멈춰야 한다.
맥미니를 재설치했다면 타겟에서 직접 읽어 대조한 뒤 `ops/known_hosts` 를 갱신한다.

```bash
ssh-keygen -lf /etc/ssh/ssh_host_ed25519_key.pub   # 맥미니에서
# 2026-09-04 확인값: SHA256:aLtltUm+bQVRLFx5HOesVJ7LeDnoSM7JXeODCVT9ohg
```

정적 파일(css/js/html)만 바꿨으면 재시작이 필요 없다. 파이썬을 고쳤으면
`launchctl kickstart -k gui/$(id -u)/com.connecteve.autodeploy` 를 따로 실행한다
— **진행 중인 작업이 있으면 끊기므로** 대시보드를 먼저 확인할 것.
