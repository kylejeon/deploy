...
설치가 끝난 서버에 **앱 버전만** 올릴 때 쓴다. 앱이 추가·제거되는 등 구조가
바뀌면 patch 가 `패치 스코프 밖` 이라며 멈춘다 — 서버는 그대로이고, 그때는
`configure` 로 반영한다.

**패치** 화면에서 적용할 서버를 고르고 ref 를 적은 뒤 `번들 만들기` 를 누른다.
그다음은 **두 단계**다.

1. 번들 생성 — 컨트롤러에서만 돈다. 끝나면 `승인 대기` 에서 멈춘다
2. 작업 상세에서 `N대에 적용 승인` 을 눌러야 서버에 반영된다

적용 대상은 **1단계에서 정한다.** 2단계는 확인만 한다. 승인 시점에 고르게 하면
번들을 다 만들어 놓고 나서야 "SSH 키가 없는 서버" 를 알게 되기 때문이다.
같은 이유로 키가 없는 서버는 화면에서 아예 고를 수 없다.

명령은 두 번에 나뉘어 나간다. 생성에는 `-l` 이 붙지 않는다 (컨트롤러 로컬 실행).

```
./bin/hubctl patch create -- -e hub_deploy_ref=v1.2.1 -e hub_deploy_ref_type=tag
  ↓ 적용 승인
./bin/hubctl patch apply -l bumin-node1,bumin-node2
```

적용에는 ref 를 넘기지 않는다 — 번들 메타가 SoT 라 값이 어긋나면 playbook 이
죽는다.

거부해도 번들은 컨트롤러에 남는다. **승인 없이 24시간이 지나면 자동 취소**되고,
이때도 번들은 남는다. 다시 하려면 패치 화면에서 새로 만든다.
